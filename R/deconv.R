...
#' @param weight Weighting scheme for hotspot-informed adjustment of spot–cell-type contributions. "Permutation": derives weights from permutation-based significance. "Scales": rescales hotspot significance within tissue identities. "NONE": applies no weighting. If `hot.pvals` is `NULL`, weighting is skipped regardless of the option. Default: Permutation.
...
estPropInSpots <- function(sp.obj, sc.obj, sc.markers, num.cells = NULL, hot.pvals = NULL, weight = c("Permutation", "Scales", "NONE"), assay = "SCT", knn = 5, intercept = TRUE) {
    base.ref <-
        {
    ...
    if (!is.null(hot.pvals) & weight != "NONE") {
        if (weight == "Permutation") {
            w.mat <- permuteWthreshold(sp.obj, hot.pvals[, cell.types, drop = FALSE], num.cells)
        } else {
    ...
    spot.prop <- future.apply::future_lapply(tar.spots, function(spot) {
        ...
        if (intercept) {
            w.coefs <- coef(MASS::rlm(Bulk ~ ., dat, maxit = 200))
        } else {
            w.coefs <- coef(MASS::rlm(Bulk ~ . - 1, dat, maxit = 200))
        }
        w.coefs[w.coefs < 0] <- 0
