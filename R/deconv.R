...
#' @param weight Logical, indicating whether to incorporate hotspot weights in the estimation. Default: TRUE.
...
estPropInSpots <- function(sp.obj, sc.obj, sc.markers, num.cells = NULL, hot.pvals = NULL, weight = TRUE, assay = "SCT", knn = 5, intercept = TRUE) {
    base.ref <-
        {
    ...
    if (!is.null(hot.pvals) & weight) {
        w.mat <- permuteWthreshold(sp.obj, hot.pvals, num.cells)
    } else {
        w.mat <- matrix(1, ncol = length(sc.markers), nrow = levels(sp.obj) %>% length(), dimnames = list(levels(sp.obj), names(sc.markers)))
    }
...
.estPropInSpots <- function(sp.score, max.cells.in.spot = 10, platform.res = "Low", ...) {
    dots <- list(...)
